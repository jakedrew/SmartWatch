...
void menuDisplay() {
 ...
 charge();
  ...
  String menuItemsSecondPage[] = { "Battery","Info", "Settings"};
...
charge();
  ...
  display.println(GyXs + "|" + GyYs + "|" + GyZs);
  display.display();
  display.clearDisplay();
}
void infoDisplay(){
...
charge();
  ...
  display.println("Built By Jake Drew");
...
charge();
  ...
  int c = (int) analogRead(A0) * (5.0 / 1023.0) / (3.3) * 100;
  String cs = String(c);
  if(c > 100){
    c = 100;
  }
  int hrs = c*1.80;
  String hrsS = String(hrs);

  display.println(cs + "% Charged");
  display.setCursor(10, 40);
  display.println(hrsS + " mins to charge");
...
charge();
  ...
  String TmpKS = String(Tmp + 273.15);
  ...
  display.println(TmpS + "C");
  
  display.setCursor(47, 35);
  display.println(TmpFS + "F");
  
  display.setCursor(44,50);
  display.println(TmpKS + "K");
...
charge();
  ...
  display.println(AcXs + "|" + AcYs + "|" + AcZs);
